...
- Software and computational setup
  ...
  - Ability to install and load common R packages (e.g., `Matrix` or similar for efficient linear algebra; potentially a package that helps work with correlation structures if desired).
...
While following the guide, users must be prepared to make several decisions that may not have unique “right” answers:
...
The final step of the task is not just accepting whatever design the algorithm proposes, but using these evaluations to reach a design that is both **statistically defensible** and **practically workable**.
