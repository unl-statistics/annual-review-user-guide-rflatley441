...
|---------|-------|--------|
| Steps 1–4: Build $X(\xi)$, $\Sigma(\xi)$, $\log\det M(\xi)$ | Linear model, design space $\mathcal{X}$, covariance structure | Criterion function to maximize |
| Step 5: Exchange search (and restarts) | Criterion function, grid, sample size $n$ | Approximate D-optimal exact design |
| Step 6: Relative D-efficiency, SEs | Optimal design, baseline design | Comparison and diagnostics |
| Step 7: Table and code | Final design | Reproducible design for the experiment |
...
Figure @fig-design-visualization helps you check that the selected design is practically reasonable (e.g., not overly clustered at one end of the grid). Below, the gray ticks show the candidate grid; filled points show the chosen design, with replications stacked vertically at each location.

```{r}
#| label: fig-design-visualization
#| eval: true
...
- **Quarto books:** For building and customizing this report, see the Quarto book documentation [@positpbcCreatingBook2024].
